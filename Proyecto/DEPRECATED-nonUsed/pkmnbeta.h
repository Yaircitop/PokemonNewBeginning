...
class pokemon{
    ...
    pokemon(char* name,int hp,int atk,int def,int spd,int crit,int tpe);
...
pokemon::pokemon():hp(0),atk(0),def(0),spd(0),crit(0),tpe(0),ohp(hp),odf(def),oatk(atk){strcpy(name,"missing");this->globalpos++;}
...
pokemon::pokemon(char* name,int hp,int atk,int def,int spd,int crit,int tpe):ohp(hp),odf(def),oatk(atk)
{strcpy(this->name,name);this->hp=hp;this->atk=atk;this->def=def;this->spd=spd;this->crit=crit;this->tpe=tpe;this->globalpos++;}
...
void pokemon::setNAME(char* nm){strcpy(name,nm);};
char* pokemon::getNAME(){return this->name;};
